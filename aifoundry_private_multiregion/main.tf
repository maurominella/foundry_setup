...
locals {
  ...
  prefixed_random_string = "${local.prefix}${local.random_suffix}"
}
...
resource "azurerm_subnet" "agents_subnet" {
  ...
  address_prefixes     = var.subnet1_agents_address_space
...
resource "azurerm_subnet" "resourcespe_subnet" {
  ...
  resource_group_name  = var.resourcegroup_name_networking
  virtual_network_name = azurerm_virtual_network.vnet.name
  address_prefixes     = var.subnet2_agents_address_space
}
...
resource "azurerm_storage_account" "storage_account" {
  ...
  name                = "storage${local.prefixed_random_string}"
...
resource "azurerm_cosmosdb_account" "cosmosdb" {
  ...
  name                = "cosmosdb${local.prefixed_random_string}"
...
resource "azapi_resource" "ai_search" {
  provider = azapi.subscription_aisearch
  type                      = "Microsoft.Search/searchServices@2025-05-01"
  name                      = "search${local.prefixed_random_string}"
...
resource "azurerm_role_assignment" "cosmosdb_operator_ai_foundry_project" {
  ...
  name                 = uuidv5("dns", "${azapi_resource.ai_foundry_project.name}${azapi_resource.ai_foundry_project.output.identity.principalId}${var.resourcegroup_name_resources}cosmosdboperator")
